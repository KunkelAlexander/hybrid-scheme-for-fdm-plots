...
for j in range(len(row)):
    for i, im in enumerate(row[j]):
        ...
        if i == 0:
            ax.annotate(row_titles[j], xy=(0.1, 0.8), xycoords="axes fraction", fontsize=8, fontweight="bold",
                        ha='left', va='center', zorder=10, color="white")

        ## Adding specific annotations
        if j == 1 and i == 1:
            ax.annotate('switch from\nfluid to wave', xy=(0.7, 0.3), xytext=(0.5, 0.5), xycoords="axes fraction",
                        arrowprops=dict(arrowstyle="->", connectionstyle="arc3"),
                        fontsize=8, ha='center', va='center', color='white')
                        #path_effects=[PathEffects.withStroke(linewidth=5, foreground='black')])
#
        if j == 2 and i == 2:
            ax.annotate('destructive\ninterference', xy=(0.4, 0.3), xytext=(0.5, 0.1), xycoords="axes fraction",
                        arrowprops=dict(arrowstyle="->", connectionstyle="arc3"),
                        fontsize=8, ha='center', va='center', color='white')
                        #path_effects=[PathEffects.withStroke(linewidth=5, foreground='black')])
