...
hubble0          = ds.hubble_constant*100/1000
...
coordinates = ts[0].arr( [ 0.00615234375, 2.21826171875, 0.464160156250 ] , 'code_length' ) 
...
for ds in ts.piter():
    ...
    halo_radius  = 0.5            # halo radius in Mpc/h --> change this value properly
    ...
    prof_mass_accumulate = yt.create_profile( sp, 'radius', fields= 'cell_mass',
                                              ...
                                              extrema = {'radius': (2e-1,1e3)}, accumulation = True)
    ...
    # collecting halo propertities
    with open("Halo_Parameter" , 'a') as file:
        ...
        q = str("{:.5e}".format(float((halo_mass*(zeta/zeta_0)**0.5)/Mmin)))
        r = str("{:.5e}".format(float((core_mass_3*current_time_a**0.5)/Mmin)))
    ...
    #         writer.writerow(['           '.join([str(float((simulation_w[i]))) , str(float(simulation_y[i])/mean_density)])])

    dens = density/background_density_0

    with open('%s_profile_data'%ds , 'w') as file:
    ...
    # # plt.plot( radius, ref1, '-' , label='r^-1' )    
    # # plt.plot( radius, ref3, '-' , label='r^-3' )
    ...
    # FileOut = 'fig_profile_density_%06d' %number+'.png'
    # plt.savefig( FileOut)
    # plt.close()
