...
for top_folder in os.listdir(directory_path):
    ...
    for item in os.listdir(top_folder_path):
        ...
        if os.path.isdir(item_path):
            match = re.search(r"'Gau_v0': ([\d.]+)", item)
            if match:
...
results_df = pd.DataFrame(data, columns=['RunName', 'Velocity', 'Error(Dens)', 'Error(Real)'])

# Save the results to a CSV file in the current directory
results_filename = "results_gaussian_wavelength.csv"
