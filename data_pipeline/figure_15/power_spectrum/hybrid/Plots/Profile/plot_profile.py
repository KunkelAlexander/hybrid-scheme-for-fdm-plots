...
for idx in range(idx_start, idx_end+1, didx):
    ...
    # read data
    df = pd.read_csv('prof/Data_%06d_profile_data'%(idx) , sep = '\t' , header = 0 )
    ...
    core_radius_2 = df_Halo_Parameter['core_radius_2'][idx]/current_time_a

    def soliton(x, core_radius):   
        return ((1.9*(float(current_time_a)**-1)*(particle_mass/10**-23)**-2*(float(core_radius)**-4))/((1 + 9.1*10**-2*(x/float(core_radius))**2)**8))*10**9/background_density_0
