...
df = pd.read_csv( 'Halo_Parameter' , sep = '\s+' , header = 0 , index_col='#')
...
for ds in ts.piter():
   ...
   coordinate_x = df['x'][58]
   coordinate_y = df['y'][58]
   coordinate_z = df['z'][58]

   sz_dens = yt.SlicePlot( ds, 'z', field, center=[coordinate_x, coordinate_y, coordinate_z] )
   # sz_dens = yt.SlicePlot( ds, 'x', field, center=[3.72421875, 4.89453125, 4.46796875] )
   sz_dens.zoom(8)
   sz_dens.set_axes_unit( 'kpc' )
   sz_dens.set_zlim( field, 1e-28 ,1.0e-23 )
   ...
   sz_dens.save('Data_%06d_Slice_z_density_color_line.png'%(num), mpl_kwargs={"dpi":dpi} )
   # sz_dens.zoom(20)
