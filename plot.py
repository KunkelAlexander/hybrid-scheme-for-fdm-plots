...
for dataset in dataset_series.piter():
    ...
    for current_axis in axes:
        ...
        axis = 'z'
        width = dataset.domain_width.min().value  # Adjust based on your dataset
        ...
        light_grey_rgba = [0.8, 0.8, 0.8, 0.5]  # Light grey with alpha for level 0
        dark_grey_rgba = [0.4, 0.4, 0.4, 0.5]  # Dark grey with alpha for level 1

        # Apply colors with alpha based on AMR level masks
        amr_image_rgba[level_0_mask, :] = light_grey_rgba
        amr_image_rgba[level_1_mask, :] = dark_grey_rgba

        # Plotting the RGBA image with alpha
        # Using the same extent as before to align with other plots
        grid[0].imshow(amr_image_rgba, origin='lower', extent=[0, width, 0, width])

        # Add annotations or other plot elements as needed
        grid[0].annotate(r"$\Re{\psi}, \Im{\psi}$ on level 1, $\rho, S$ on level 0", (0.5, 1.05),
                        xycoords='axes fraction', ha='center', va='bottom', fontsize=12, c="white",
                        arrowprops=dict(arrowstyle="<->"))

        # Add the overlay to the last subplot
        #grid[3].imshow(Z, extent=[0, 1, 0, 1], origin='lower', alpha=0.5, cmap='gray')
