...
def set_config_flag(filename, flag_name, new_value):
    """
    Update the value of a given flag in a configuration file.
    ...
    for i, line in enumerate(lines):
        if line.strip().startswith(flag_name):
            parts = line.split()
            # Find the index of the comment start (#) if present
            comment_start = len(parts)
            for j, part in enumerate(parts):
                if part.startswith('#'):
                    comment_start = j
                    break
            # Update the value before the comment starts
            parts[comment_start - 1] = str(new_value)
            lines[i] = '   '.join(parts[:comment_start]) + ('   ' + ' '.join(parts[comment_start:]) if comment_start < len(parts) else '') + '\n'
            updated = True
            break  # Assuming only one occurrence of the flag; remove this if multiple updates are needed

    if updated:
        with open(filename, 'w') as file:
            file.writelines(lines)
    else:
        print(f"Warning: Flag '{flag_name}' not found in the file.")

def execution( **kwargs ):
    cwd               = os.getcwd()
    record_parameters = kwargs["record"]
    exe               = kwargs["exe"]
    record_parameters["OPT__REF_FLU_INT_SCHEME"] = record_parameters["OPT__FLU_INT_SCHEME"]

    #Ensure that OPT__REF_FLU_INT_SCHEME = OPT__FLU_INT_SCHEME
    set_config_flag("Input__Parameter", "OPT__REF_FLU_INT_SCHEME", record_parameters["OPT__FLU_INT_SCHEME"])
    ...
    # Store Data_000000 with refinement
    set_config_flag("Input__Parameter", "MAX_LEVEL", 1)
    set_config_flag("Input__Parameter", "OPT__INIT", 1)
    subprocess.run([kwargs["exe"]], capture_output=False)

    #Rename reference files
    for file in glob.glob(r'VortexPairLinear*'):
        shutil.move(os.path.join(cwd, file), os.path.join(cwd, os.path.join(dest_dir, file + "_hr")))

    for file in glob.glob(r'Data*'):
        shutil.move(os.path.join(cwd, file), os.path.join(cwd, os.path.join(dest_dir, file + "_hr")))

    # Store Data_000000 without refinement
    set_config_flag("Input__Parameter", "MAX_LEVEL", 0)
    set_config_flag("Input__Parameter", "OPT__INIT", 1)
    subprocess.run([kwargs["exe"]], capture_output=False)

    #Rename reference files
    for file in glob.glob(r'VortexPairLinear*'):
        shutil.move(os.path.join(cwd, file), os.path.join(cwd, file + "_lr"))

    for file in glob.glob(r'Data*'):
        shutil.move(os.path.join(cwd, file), os.path.join(cwd, file + "_lr"))


    #Enable refinement, read IC from Data_000000_ref to force interpolation
    shutil.copy("Data_000000_lr", "RESTART")

    set_config_flag("Input__Parameter", "MAX_LEVEL", 1)
    set_config_flag("Input__Parameter", "OPT__INIT", 2)
    #Run to check interpolation
    subprocess.run([kwargs["exe"]], capture_output=False)
...
#loop_paras1  = {"OPT__FLU_INT_SCHEME":[4, 6, 8], "OPT__INT_PHASE":[0, 1]}                     # the parameters change as given list
loop_paras1  = {"OPT__FLU_INT_SCHEME":[4, 6, 8], "OPT__INT_PHASE":[0, 1]}                     # the parameters change as given list
...
const_paras2 = {}
#loop_paras2  = {"VorPairLin_kx":np.array([1, 2, 3, 4, 6, 9, 13, 19, 27, 40, 57, 83, 119, 172, 247, 355, 512])}#np.arange(1, 60, 2)}
loop_paras2  = {"VorPairLin_kx":np.array([1, 2, 3, 4, 5, 7, 9, 11, 14, 19, 24, 30, 39, 50, 64])}
